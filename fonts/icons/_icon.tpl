...
<html lang="ru">
    ...
    <body class="icons">
        <table border="1" width="100%">
            ...
            {{#glyphs}}
                <tr>
                    <td class="icon"><div class="i-font-{{name}}"></div></td>
                    <td>i-font-{{name}}</td>
                    <td>{{options.fontId}}-char({{name}}) => '\{{string}}'</td>
                    <td class="code">
                            :before - @include {{options.fontId}}({{name}});<br />
                            :after  - @include {{options.fontId}}({{name}}, after);
                    </td>
                </tr>
            {{/glyphs}}
            </tbody>
